...
export class WoCurtain extends SwitchbotDevice {
	...
	static async parseServiceData(
		...
		if (![5, 6].includes(serviceData.length)) {
			emitLog(
				"debugerror",
				`[parseServiceDataForWoCurtain] Buffer length ${serviceData.length} !== 5 or 6!`
			);
			return null;
		}
		...
		// const batteryLevel =
		const { data: bufferData } = JSON.parse(JSON.stringify(serviceData)) as {
			data: number[];
		};

		// console.log(bufferData);

		const battery = bufferData[2];
		const position = bufferData[3] > 100 ? bufferData[3] - 128 : bufferData[3];
		const inMotion = bufferData[3] > 100;
		// 225 - increase %
		// 228 - decrease %

		// const model = (serviceData.subarray(0, 1).toString("utf8") as string)
	...
	public async operateCurtain(bytes: number[]): Promise<void> {
		...
		await this.commandWithoutResponse(reqBuf);
	}
}
