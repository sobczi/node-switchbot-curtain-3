...
export class SwitchBotBLE extends EventEmitter {
	...
	private async initialize(params?: Params): Promise<void> {
		try {
			...
			this.log(
				"error",
				`Failed to import noble: ${JSON.stringify(e.message ?? e)}`
			);
		}
	}
	...
	public async validate(
		...
		if (!valid) {
			this.log(
				"error",
				`parameterChecker: ${JSON.stringify(parameterChecker.error!.message)}`
			);
	...
	public async discover(params: Params = {}): Promise<SwitchbotDevice[]> {
		...
		const finishDiscovery = async () => {
			...
			if (this.noble) {
				...
				try {
					...
					this.log(
						"error",
						`discover stopScanningAsync error: ${JSON.stringify(e.message ?? e)}`
					);
				}
			}
		...
		return new Promise<SwitchbotDevice[]>((resolve, reject) => {
			this.noble.on(
				"discover",
				async (peripheral: NobleTypes["peripheral"]) => {
			...
			this.noble
				.startScanningAsync(PRIMARY_SERVICE_UUID_LIST, false)
				.then(() => {
					timer = setTimeout(
						async () => resolve(await finishDiscovery()),
						p.duration
					);
				})
				.catch(reject);
		...
		if (ad && (await this.filterAd(ad, id, model)) && this.noble) {
			switch (ad.serviceData.model) {
				case SwitchBotBLEModel.Bot:
					return new WoHand(peripheral, this.noble);
				case SwitchBotBLEModel.Curtain:
				case SwitchBotBLEModel.Curtain3:
					return new WoCurtain(peripheral, this.noble);
				case SwitchBotBLEModel.Humidifier:
					return new WoHumi(peripheral, this.noble);
				case SwitchBotBLEModel.Meter:
				case SwitchBotBLEModel.MeterPlus:
					return new WoSensorTH(peripheral, this.noble);
				case SwitchBotBLEModel.Hub2:
					return new WoHub2(peripheral, this.noble);
				case SwitchBotBLEModel.OutdoorMeter:
					return new WoIOSensorTH(peripheral, this.noble);
				case SwitchBotBLEModel.MotionSensor:
					return new WoPresence(peripheral, this.noble);
				case SwitchBotBLEModel.ContactSensor:
					return new WoContact(peripheral, this.noble);
				case SwitchBotBLEModel.ColorBulb:
					return new WoBulb(peripheral, this.noble);
				case SwitchBotBLEModel.CeilingLight:
				case SwitchBotBLEModel.CeilingLightPro:
					return new WoCeilingLight(peripheral, this.noble);
				case SwitchBotBLEModel.StripLight:
					return new WoStrip(peripheral, this.noble);
				case SwitchBotBLEModel.PlugMiniUS:
				case SwitchBotBLEModel.PlugMiniJP:
					return new WoPlugMini(peripheral, this.noble);
				case SwitchBotBLEModel.Lock:
					return new WoSmartLock(peripheral, this.noble);
				case SwitchBotBLEModel.LockPro:
					return new WoSmartLockPro(peripheral, this.noble);
				case SwitchBotBLEModel.BlindTilt:
					return new WoBlindTilt(peripheral, this.noble);
				default:
					return new SwitchbotDevice(peripheral, this.noble);
			}
		}
		return null;
	}
	...
	private async filterAd(ad: Ad, id: string, model: string): Promise<boolean> {
		...
		if (
			id &&
			ad.address.toLowerCase().replace(/[^a-z0-9]/g, "") !==
	...
	public async startScan(params: Params = {}): Promise<void> {
		...
		try {
			...
			this.log(
				"error",
				`startScanningAsync error: ${JSON.stringify(e.message ?? e)}`
			);
		}
	}
	...
	public async stopScan(): Promise<void> {
		...
		try {
			...
			this.log(
				"error",
				`stopScanningAsync error: ${JSON.stringify(e.message ?? e)}`
			);
		}
	}
	...
	public async wait(msec: number): Promise<void> {
		if (typeof msec !== "number" || msec < 0) {
			throw new Error(
				"Invalid parameter: msec must be a non-negative integer."
			);
		}
