...
export class Advertising {
	...
	static async parse(
		...
		if (
			!Advertising.validateBuffer(serviceData) ||
			!Advertising.validateBuffer(manufacturerData)
		) {
			return null;
		}

		const model = serviceData.subarray(0, 1).toString("utf8");
		const sd = await Advertising.parseServiceData(
			model,
			serviceData,
			manufacturerData,
			emitLog
		...
		emitLog(
			"debug",
			`[parseAdvertising.${peripheral.id}.${model}] return ${JSON.stringify(data)}`
		);
		return data;
	}
	...
	public static async parseServiceData(
		...
		switch (model) {
			...
			case SwitchBotBLEModel.Curtain3:
				return WoCurtain.parseServiceData(
					serviceData,
					manufacturerData,
					emitLog
				);
			...
			case SwitchBotBLEModel.OutdoorMeter:
				return WoIOSensorTH.parseServiceData(
					serviceData,
					manufacturerData,
					emitLog
				);
			...
			case SwitchBotBLEModel.Lock:
				return WoSmartLock.parseServiceData(
					serviceData,
					manufacturerData,
					emitLog
				);
			case SwitchBotBLEModel.LockPro:
				return WoSmartLockPro.parseServiceData(
					serviceData,
					manufacturerData,
					emitLog
				);
			case SwitchBotBLEModel.BlindTilt:
				return WoBlindTilt.parseServiceData(
					serviceData,
					manufacturerData,
					emitLog
				);
			default:
				emitLog(
					"debug",
					`[parseAdvertising.${model}] return null, model "${model}" not available!`
				);
				return null;
		}
	}
