...
export class SwitchbotDevice extends EventEmitter {
	...
	constructor(
		...
		Advertising.parse(peripheral, this.log.bind(this)).then((ad) => {
			...
			this.deviceModelName =
				(ad?.serviceData.modelName as SwitchBotBLEModelName) ?? "";
			this.deviceFriendlyName =
				(ad?.serviceData.modelFriendlyName as SwitchBotBLEModelFriendlyName) ??
				"";
		});
	}
	...
	set onConnectHandler(func: () => Promise<void>) {
		if (typeof func !== "function") {
			throw new TypeError(
				"The `onConnectHandler` must be a function that returns a Promise<void>."
			);
		}
	...
	set onDisconnectHandler(func: () => Promise<void>) {
		if (typeof func !== "function") {
			throw new TypeError(
				"The `onDisconnectHandler` must be a function that returns a Promise<void>."
			);
		}
	...
	private async internalConnect(): Promise<void> {
		if (this.noble._state !== "poweredOn") {
			throw new Error(
				`The Bluetooth status is ${this.noble._state}, not poweredOn.`
			);
		}
	...
	public async getCharacteristics(): Promise<Chars> {
		const timer = setTimeout(() => {
			throw new Error(
				"Failed to discover services and characteristics: TIMEOUT"
			);
		}, 5000);
	...
	async getDeviceName(): Promise<string> {
		...
		if (!this.characteristics?.device) {
			throw new Error(
				`The device does not support the characteristic UUID 0x${CHAR_UUID_DEVICE}.`
			);
		}
	...
	async commandWithoutResponse(reqBuf: Buffer): Promise<void> {
